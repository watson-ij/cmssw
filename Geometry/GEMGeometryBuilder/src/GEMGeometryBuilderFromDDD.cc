				  ...
				  const MuonDDDConstants& muonConstants )
{
  ...
  while (doSuper) {
    ...
    if (fv.logicalPart().name().name() == "ME0Box") {
      ...
      GEMNumberingScheme me0NumCh(muonConstants); 
      int rawId = me0NumCh.baseNumberToUnitNumber(mdddnumCh.geoHistoryToBaseNumber(fv.geoHistory()));

      GEMDetId detId = GEMDetId(rawId);
      if (detId.station() != 0) {
	std::cout << "::WARNING:: Bad ME0 GEMDetId " << detId.region() << " " << detId.ring() << " " << detId.station() << std::endl;
      }
      ...
      while (doChambers) {
        ...
        while (doEtaPart) {
          ...
          GEMDetId detId = GEMDetId(rawId);
	  if (detId.station() != 0) {
	    std::cout << "::WARNING:: Bad ME0 eta partition GEMDetId " << detId.region()
		      << " " << detId.ring()
		      << " " << detId.station()
		      << " " << detId.layer()
		      << " " << detId.chamber()
		      << " " << detId.roll()
		      << std::endl;
	    auto num = mdddnum.geoHistoryToBaseNumber(fv.geoHistory());
      ...
      if (detId.layer() == 1 ) {
        ...
        superChambers.push_back(gemSuperChamber);
	if (detId.station() == 0) {
	  std::cout << "::WARNING:: Bad GEM GEMDetId " << detId.region() << " " << detId.ring() << " " << detId.station() << std::endl;
	}
      }
